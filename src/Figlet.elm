...
run pipedInput args =
    let
        input =
            Maybe.withDefault (Maybe.withDefault "" (List.head args)) pipedInput

        parsedFont =
            Parser.run fontParser standard
    in
    case parsedFont of
        Err _ ->
        ...
        Ok font ->
            Screen.batch
                (toLines font (Debug.log "input" input)
...
fontParamsParser =
    succeed FontParams
        ...
        |= oneOf
            [ Parser.map Just int
            , Parser.map (always Nothing) (chompIf ((==) '\n'))
            ]
